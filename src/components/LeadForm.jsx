...
const LeadForm = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    if (status === 'failed') {
      onClose(); // Close the form on failure
      toast.error('Failed to submit lead');
    }
  }, [status, onClose]);
  ...
  return (
    <div className="fixed z-50 inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-6 md:p-14 rounded-lg shadow-lg w-11/12 md:w-3/6 relative">
        <button onClick={onClose} className="absolute top-4 right-4 md:top-6 md:right-5 text-2xl md:text-3xl hover:text-gray-700">
          <IoClose />
        </button>
        <hr className="h-px mb-6 md:mb-10 bg-gray-100 border-0 dark:bg-gray-300" />
        <form onSubmit={handleSubmit}>
          <div className="flex flex-col md:flex-row md:space-x-8">
            <div className="mb-4 w-full md:w-3/6">
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="Enter Name"
                className="w-full px-3 py-2 md:py-4 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:border-blue-300"
                required
              />
            </div>
            <div className="mb-4 w-full md:w-3/6">
          ...
          <div className="mb-4">
            <label className="inline-flex items-center">
              <input
                ...
                checked={formData.agreedToTerms}
                onChange={handleChange}
                className="focus:ring focus:border-blue-300"
                required
              />
              <span className="ml-2">
                I agree to all the{" "}
          ...
          <div className="text-center">
            <button
              type="submit"
              className="w-full bg-blue-500 text-white px-4 py-2 md:py-4 rounded-lg hover:bg-blue-700 focus:outline-none focus:bg-blue-700 flex justify-center items-center"
            >
              Submit
              {status === 'loading' && <FaSpinner className="ml-2 animate-spin" />}
            </button>
